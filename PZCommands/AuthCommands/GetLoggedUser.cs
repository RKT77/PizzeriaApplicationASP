...
namespace PizzeriaCommands.AuthCommands
{
    public class GetLoggedUser : BaseCommand, IGetLoggedUser
    {
        public GetLoggedUser(PizzeriaContext ctx) : base(ctx)
        {
        }

        public LoggedUser Execute(LoginRequest loginRequest)
        {
            var response = this.context.Attendants.AsQueryable().Include(p=>p.Role)
